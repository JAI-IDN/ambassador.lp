...
| 2 | Hero | 価値提案見出し + 主CTA + ログイン + 夜景イラスト写真 + 浮遊カード |
| 3 | Process strip | 5ステップ俯瞰（グラスモーフィズム） |
| 4 | Problems（お悩み） | 3つの課題提示（見出しはコーラル系アクセント） |
| 5 | How it works（仕組み） | 5ステップのフロー詳細 |
| 6 | Activities（活動例） | 4カテゴリのカード |
| 7 | Points usage | スマホUI + ポイント活用 |
| 8 | CTA（桜バンド） | 桜イラスト写真＋最終後押し＋主CTA |
| 9 | Footer | ブランド・規約・著作権 |

> アイコンは全て**インラインSVG**（絵文字不使用）。

## 7. 画像アセット

提供デザインの写真に対応するビジュアルを、外部依存ゼロの**ベクターイラスト（SVG）**として `assets/` に内製。実写差し替えも容易（同名で `.jpg` を配置し `src` を変更するだけ）。

| ファイル | 用途 | 内容 |
|---|---|---|
| `assets/hero.svg` | ヒーロー | 東京タワー夜景＋スマホを持つ人物のバックライトシルエット＋月・街並み |
| `assets/sakura.svg` | CTAバンド | 桜の枝・舞う花びら・笑顔の若者3人 |

> 外部リソースは GoogleFonts の `<link>`（`preconnect` + `font-display:swap`）のみ。画像はローカル相対参照のためダブルクリック（`file://`）でも表示可。
